...
short unsigned int Tree_Insert(Node* t, char* k, short unsigned int si, TVal val){
  ...
  BRTree* stack[i+i+1]; // +1 for NULL elem
  *stack = t->head;
  i = 0;
  // search a place
  while(stack[i]){
  ...
  // ballance
  stack[i] = n;
  while(i && !stack[i-1]->color){
    if(stack[i-2]->right == stack[i-1]){
      if(stack[i-2]->left && !stack[i-2]->left->color){
        stack[i-1]->color = stack[i-2]->left->color = 1;
        i-=2;
        stack[i]->color = 0;
        continue;
      }else if(stack[i-1]->left == stack[i]){
        stack[i-2]->right = Right_Rotate(stack[i-1]);
      }
      i-=2;
      if(i){
        if(stack[i-1]->right == stack[i]){
          stack[i-1]->right = Left_Rotate(stack[i]);
          return 0;
        }else{
          stack[i-1]->left = Left_Rotate(stack[i]);
          return 0;
        }
      }else{
        t->head = Left_Rotate(stack[i]);
        return 0;
      }
    }else{
      if(stack[i-2]->right && !stack[i-2]->right->color){
        stack[i-1]->color = stack[i-2]->right->color = 1;
        i-=2;
        stack[i]->color = 0;
        continue;
      }else if(stack[i-1]->right == stack[i]){
        stack[i-2]->left = Left_Rotate(stack[i-1]);
      }
      i-=2;
      if(i){
        if(stack[i-1]->right == stack[i]){
          stack[i-1]->right = Right_Rotate(stack[i]);
          return 0;
        }else{
          stack[i-1]->left = Right_Rotate(stack[i]);
          return 0;
        }
      }else{
        t->head = Right_Rotate(stack[i]);
        return 0;
      }
    }
  }
...
short unsigned int Tree_Delete(Node* t, char* k){
  ...
  BRTree *h, *fh, *grandpa, *father, *my = t->head;
  h = father = grandpa = NULL;
  bool gv = 0;
  bool fv = 0;

  while(my){
    short i = strcmp(k, my->key);
    if(my->color){
      if(i > 0){
        if(!my->right || my->right->color){
          if(my->left && !my->left->color){
            if(father && !fv){
              father = father->left = Right_Rotate(my);
            }else if(fv){
              father = father->right = Right_Rotate(my);
            }else{
              father = t->head = Right_Rotate(my);
            }
            grandpa = father;
            gv = fv;
            father = my;
        ...
        if(!my->left || my->left->color){
          if(my->right && !my->right->color){
            if(father && !fv){
              father = father->left = Left_Rotate(my);
            }else if(fv){
              father = father->right = Left_Rotate(my);
            }else{
              father = t->head = Left_Rotate(my);
            }
            if(!i){
              fh = father;
              h = my;
      ...
      if((!my->right || my->right->color) && (!my->left || my->left->color) && father && father->right && father->left){
          BRTree* bro;
          if(fv){
            bro = father->left;
          }else{
          ...
          if((!bro->right || bro->right->color) && (!bro->left || bro->left->color)){
            father->color = 1;
            bro->color = my->color = 0;
          }else{
            if(grandpa && !gv){
              if(fv){
                if(bro->right && !bro->right->color){
                  father->left = Left_Rotate(bro);
                }
                grandpa->left = Right_Rotate(father);
              }else{
                if(bro->left && !bro->left->color){
                  father->right = Right_Rotate(bro);
                }
                grandpa->left = Left_Rotate(father);
              }
              my->color = grandpa->left->color = 0;
              grandpa->left->right->color = grandpa->left->left->color = 1;
            }else if(gv){
              if(fv){
                if(bro->right && !bro->right->color){
                  father->left = Left_Rotate(bro);
                ...
                grandpa->right = Left_Rotate(father);
              }
              my->color = grandpa->right->color = 0;
              grandpa->right->right->color = grandpa->right->left->color = 1;
            }else{
              if(fv){
                if(bro->right && !bro->right->color){
                  father->left = Left_Rotate(bro);
                }
                t->head = Right_Rotate(father);
              }else{
                if(bro->left && !bro->left->color){
                  father->right = Right_Rotate(bro);
                }
                t->head = Left_Rotate(father);
              }
              my->color = t->head->color = 0;
              t->head->right->color = t->head->left->color = 1;
            }
          }
      }
    }
  ...
  if(h){
    ...
    father->color = h->color;
    father->right = h->right;
    father->left = h->left;
    free(h);
    --t->size;
  }else{
